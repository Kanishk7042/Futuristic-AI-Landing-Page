        ...
        <div className="h-full hidden lg:block">
            ...
            <nav className="h-full">
              {navItems.map(({name,href})=>(
                <a href={href} key={href} className="px-10 h-full relative font-bold text-xs tracking-widest text-gray-400 uppercase inline-flex items-center before:content-[''] before:absolute before:bottom-0 before:h-2 before:w-px before:bg-gray-200/20 before:left-0  last:after:absolute last:after:bottom-0 last:after:h-2 last:after:w-px last:after:bg-gray-200/20 last:after:right-0   " onClick={(e)=>{
                  e.preventDefault();
    ...
    <div className="fixed top-18 left-0 bottom-0 right-0 bg-gray-950 z-30 overflow-hidden">
      ...
      <div className="container h-full ">
        ...
        <nav className="flex flex-col items-center gap-4 py-8 h-full justify-center">
          ...
          {navItems.map(({name,href})=>(
            <a href={href} key={name} className="text-gray-400 uppercase tracking-widest font-bold text-xs h-10" onClick={(e)=>{
              e.preventDefault();
              const element=document.querySelector(href);
              if(element){
