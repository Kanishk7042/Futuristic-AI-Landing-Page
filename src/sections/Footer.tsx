...
export const Footer = () => {
  return <footer className=" border-t border-[var(--color-border)]">
      ...
      <div className="flex justify-center gap-6">
        {socialLinks.map((link)=>(
         <a href={link.href}  key={link.name}>
          <div className="size-10 rounded-e-full bg-gray-900 inline-flex items-center justify-center">
            <FontAwesomeIcon icon={link.icon} className="size-4 "/>
          </div>
          </a>
        ))}
      </div>
      <p className="text-gray-500 text-sm">&copy; Kanishk Aswal, All rights reserved.</p>
    </div>
