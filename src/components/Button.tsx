...
export type ButtonProps={variant?:'primary'| 'secondary'|'tertiary';
    block?:boolean;
...
const classes = cva('text-xs tracking-widest uppercase font-bold h-10 px-6 rounded-lg', {
  ...
  export const Button=(props:ButtonProps)=>{
   ...
   animate(angle,angle.get()+360,{
    ...
    return(
    <motion.button onMouseEnter={()=>setIsHovered(true)}  onMouseLeave={()=>setIsHovered(false)} className={classes({...otherProps,variant,className})}
    style={
      variant==='primary' ?{
